...
// NOTA: evitamos variables llamadas 'location' — es reservada en el browser
...
describe("🌊 Ocean-Sense Network", () => {
  ...
  it("Envía una lectura oceánica (agua limpia)", async () => {
    const idx = new BN(0);
    ...
    console.log("   Temp:", r.temperature / 100, "°C");
    console.log("   Salinidad:", r.salinity / 100, "PSU");
    console.log("   Oleaje:", r.waveHeight / 100, "m");
    console.log("   Contaminación:", r.pollutionLevel, "(0=limpio)");
    console.log("   Recompensa:", r.rewardLamports.toString(), "lamports");
  ...
  it("Detecta contaminación crítica y da mayor recompensa", async () => {
    const idx = new BN(1);
    ...
    console.log("🚨 Alerta crítica | tx:", tx);

    const r = await program.account.oceanReading.fetch(alertPda);
    console.log("   Nivel contaminación:", r.pollutionLevel, "(3=crítico)");
    console.log("   Recompensa especial:", r.rewardLamports.toString(), "lamports");
  ...
  it("Desactiva la boya (mantenimiento)", async () => {
    ...
    console.log("⛔ Boya desactivada");
  });

  it("Rechaza lecturas cuando la boya está inactiva", async () => {
    const idx = new BN(2);
    ...
    try {
      await program.methods
        ...
        .rpc();
      assert(false, "Debió fallar con boya inactiva");
    } catch (err: any) {
      const msg: string = err.message ?? "";
      const ok = msg.includes("BuoyNotActive") || msg.includes("6002");
      console.log("✅ Error esperado:", ok ? "BuoyNotActive ✓" : msg);
      assert(ok);
    }
  });

  it("Reactiva la boya y muestra estadísticas finales", async () => {
    await program.methods
      .toggleBuoy(true)
      .accounts({ buoy: buoyPda, operator: operator.publicKey })
      .rpc();
    ...
    console.log(
      "   Recompensas:",
      acct.totalRewards.toString(),
      "lamports =",
      (acct.totalRewards.toNumber() / 1e9).toFixed(6),
      "SOL"
    );
